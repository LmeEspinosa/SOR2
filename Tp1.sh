...
Bandera=false
Menu='Seleccione:'
opcion=("Info MBR" "Leer BootSector" "Leer FileSystem" "Leer Datos de Archivos" "Ver MBR en Bless" "Salir")
select op in "${opcion[@]}"
do
if ! $Bandera; then
   echo "Comencemos" 
   ...
   Bandera=true
    ...
    case $op in
        ...
        "Leer Datos de Archivos")
            echo "Seleccionaste Leer Datos de Archivos"
                 gcc read_file.c -o readfile
                 ./readfile > Tp1Log.txt; cat Tp1Log.txt
            ;;
        "Ver MBR en Bless")
